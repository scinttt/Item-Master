...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                if isSearching {
                    ...
                    ForEach(categories) { category in
                        NavigationLink(destination: LazyView(CategoryDetailView(category: category))) {
                            HStack {
                                Text(category.name)
                                Spacer()
                        ...
                        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                            Button(role: .destructive) { prepareDelete(category) } label: { Label("删除", systemImage: "trash") }
                            Button { prepareRename(category) } label: { Label("编辑", systemImage: "pencil") }.tint(.orange)
                        }
                    }
                    ...
                    if isAddingCategory {
                        TextField("分类名称", text: $newCategoryName)
                            ...
                            .onChange(of: isTextFieldFocused) { _, isFocused in
                                if !isFocused && isAddingCategory { saveNewCategory() }
                            }
                    } else {
                        Button {
                            ...
                            Label("添加分类", systemImage: "plus").foregroundStyle(.tint)
                        }
                    }
                }
            }
            ...
            .alert("修改汇率", isPresented: $showRateAlert) {
                TextField("1 USD = ? CNY", text: $rateInput).keyboardType(.decimalPad)
                Button("取消", role: .cancel) {}
                Button("确定") { if let newRate = Double(rateInput), newRate > 0 { exchangeRate = newRate } }
            } message: { Text("请输入 1 美元兑换人民币的汇率。") }
            .alert("重命名分类", isPresented: $showRenameAlert) {
                TextField("新名称", text: $renameInput)
                Button("取消", role: .cancel) {}
                Button("保存") { if let category = categoryToRename { category.name = renameInput.trimmingCharacters(in: .whitespaces) } }
            }
    ...
    private func prepareDelete(_ category: Category) {
        ...
        if !category.items.isEmpty { showDeleteRestrictedAlert = true }
